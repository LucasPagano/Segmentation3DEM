...
sys.path.insert(0, './PyTorch/Models/ResUnet-a/')
import ResUnetA
...
"""
model = ResUnetA.ResUnetA(Inputs=1, Depth=3, FeatureMaps=16, Activations="relu", FirstLastBlock="ResBlock", Dilations=(1,13,12),
				...
				DropOutPosition="Block", DropOut=[0.11, 0.07, 0.05], OutputActivations=["sigmoid", "sigmoid", "sigmoid", "sigmoid"], ConcatenateOutputs=True)
...
model = UNet.UNet(Inputs=1, Depth=4, FeatureMaps=8, KernelSize=3, Activations="relu", IntermediateActivation=False,
				...
				DropOut=[0.13, 0.11, 0.07, 0.05], Outputs=1, OutputActivation="sigmoid")
"""
...
inputs  = "/Users/firetiti/Downloads/CyclicIF/IF Manual Segmentation/BR1506-A015/001 - Cropped/Originals Stretched/"
outputs = "/Users/firetiti/Downloads/CyclicIF/IF Manual Segmentation/BR1506-A015/001 - Cropped/Outs 01/"
...
InputsNormalizer  = Normalizers.CenterReduce()
#InputsNormalizer  = Normalizers.CenterReduceGlobal(RemoveOutliers=0)
...
dl = generator.PyTorch(BatchSize, InputsNormalizer=InputsNormalizer, OutputsNormalizers=OutputsNormalizers, Workers=5)
"""
...
model = torch.load("./Model_Work0.pt", map_location="cpu")
model.eval()
model = model.to(device)
...
segmentator.PyTorch(inputs, model, device, CropSize=dim, BorderEffectSize=73, InputNormalizer=InputsNormalizer, BatchSize=2,
					OutputsNormalizers=OutputsNormalizers, ResultsDirPath="./Results/Segmentator/")
segmentator = None
