...
def ConvModule(in_channels, out_channels, kernel_size, conv_cfg, norm_cfg, act_cfg, inplace=False):
...
class SwinModule(nn.Module):
    def __init__(self,
        ...
        self.swin_blocks = BasicLayer(dim=dim,
                 depth=depth,
                 num_heads=num_heads,
                 window_size=window_size,
                 mlp_ratio=mlp_ratio,
    ...
    def forward(self, x):
        ...
        x, _, _, _, _, _ = self.swin_blocks(x, H, W)
...
class FPNT(nn.Module):
    ...
    Example:
        ...
        >>> self = FPNT(in_channels, 11, len(in_channels)).eval()
...
if __name__=='__main__':
    ...
    self = FPT1(in_channels, 256, len(in_channels), window_size=12).eval()
    outputs = self.forward(inputs)
    for i in range(len(outputs)):
        print(f'outputs[{i}].shape = {outputs[i].shape}')
    ...
    inputs = [torch.rand(1, 96, 128, 172),
              torch.rand(1, 192, 64, 86),
              torch.rand(1, 384, 32, 43),
              torch.rand(1, 768, 16, 22)]


    self = FPT(in_channels, 256, len(in_channels)).eval()
    outputs = self.forward(inputs)
    for i in range(len(outputs)):
        print(f'outputs[{i}].shape = {outputs[i].shape}')
