...
from PyTorch_Models.CPS.code.utils.utils_senformer import SwinTransformer, FPNT
import torch
...
class SenFormer(nn.Module):

    def __init__(self, num_heads, branch_depth, in_chans, num_classes, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop=0.,
                 attn_drop=0., drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, eps=1.e-15,
                 align_corners=False, ignore_index=255, swin_tiny=False, **kwargs):
        super(SenFormer, self).__init__()
        # Backbone
        if swin_tiny:
            self.backbone = SwinTransformer(in_chans=in_chans, embed_dim=96,
                                            depths=[2, 2, 6, 2],
                                            num_heads=[3, 6, 12, 24],
                                            window_size=7,
                                            drop_path_rate=0.3,
        ...
        # FPNT
        self.neck = FPNT(in_channels=self.in_channels,
                         out_channels=512,
                         num_outs=4,
                         depth_swin=1,
                         num_heads=8)
    ...
    def forward(self, inputs):
        ...
        for i in range(0, len(self.in_channels)):
            ...
            learner_pred = F.interpolate(learner_pred, size=x[0].shape[2:], mode='bilinear',
    ...
    def forward_test(self, inputs):
        ...
        return F.interpolate(ensemble_pred, size=inputs.shape[2:], mode="bilinear")
    ...
    def losses(self, seg_logit, seg_label):
        ...
        ensemble_pred = F.interpolate(input=ensemble_pred, size=seg_label.shape[2:], mode='bilinear',
                                      align_corners=self.align_corners)
        ## Learners predicitions
        seg_log_logit = [F.interpolate(input=logit_outputs[i], size=seg_label.shape[2:], mode='bilinear',
                                       align_corners=self.align_corners)
...
if __name__ == '__main__':
    ...
    W = 512
    num_classes = 2
    model = SenFormer(num_heads=8, branch_depth=6, in_chans=1, num_classes=num_classes, mlp_ratio=4., qkv_bias=True,
                      qk_scale=None, drop=0.,
                      attn_drop=0., drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, eps=1.e-15,
                      align_corners=False)
    inputs = torch.randn((B, 1, H, W))
    inputs2 = torch.randn((B*2, 1, H, W))
    labels = torch.randint(low=0, high=2, size=(B, 1, H, W)).long()


    with torch.no_grad():
        outputs = model(inputs)
        outputs2 = model(inputs2)
        ensemble_l = torch.cat((outputs[1], outputs2[1]), dim=0)
        logits_l = [torch.cat((outputs[0][i], outputs2[0][i]),dim=0) for i in range(len(outputs[0]))]

        loss = model.losses(outputs, labels)
        print('loss:', loss)
        print("forward eval:", model.forward_test(inputs).shape)
