...
from PyTorch_Models.CEECNET.dset import SegmentationDataset, UnsupervisedSegmentationDataset
                ...
                if i_batch == 0:
                    for j, img in enumerate(l_batch):
                        if j < 2:
                            ...
                            pred_msk = detach_to_numpy(torch.where(outputs_soft[j] > 0.5, 1, 0).squeeze()).astype(
                                np.uint8)
                            to_log_img = wb_mask(bg_img=img, pred_mask=pred_msk, true_mask=gt_msk,
