...
def sliding_window(x, model, config):
    ...
    if HPP_DEFAULT.get("run_ids") is not None:
        ...
        outs = {r: [] for r in range(len(run_ids))}
        for j, run_id in enumerate(run_ids):
            model, config = get_model(run_id)
            ...
            for i in range(0, patches.size(0), batch_size):
                ...
                if config.get("ceecnet"):
                    ...
                    outs[j].append(detach_to_numpy(model(ins)[0]))
                elif config.get("model_smp"):
                    outs[j].append(detach_to_numpy(model(ins)))
                else:
                    outputs_tanh, outputs = model(ins)
                    outs[j].append(detach_to_numpy(torch.sigmoid(outputs)))
        outs_final = {i: np.concatenate(outs[i], axis=0) for i in outs}
        final_array = np.array(list(outs_final.values()))
        if final_array.shape[0] > 1:
            outs = np.mean(final_array, axis=0)
        else:
            outs = final_array[0]
    else:
        outs = []
        ...
        for i in range(0, patches.size(0), batch_size):
            ...
            if config.get("ceecnet"):
                ...
                outputs_tanh, outputs = model(ins)
                outs.append(torch.sigmoid(outputs))
        outs = np.concatenate(outs, axis=0)

    segmentation_masks = torch.tensor(outs).to(device)  # [nb_patches_all, nbclasses, dimensions_input, dimensions_input]
...
def run_eval(run_id=None):
    ...
    elif HPP_DEFAULT.get("run_ids"):
        HPP_DEFAULT.run_id = HPP_DEFAULT.run_ids[0]
    ...
    model, config = get_model(HPP_DEFAULT.run_id)
...
def get_model(run_id):
    ...
    checkpoint = torch.load("./models/{}/{}".format(run_id, file_name), map_location=device)
    if config.get("ceecnet"):
        try:
